...
if ($payment_id > 0) {
    ...
    $payment = $database->getSingle($query, [$payment_id, $user_id]);
} elseif ($booking_id > 0) {
    $query = "SELECT p.*, b.id as booking_id, b.booking_code, b.check_in, b.check_out, b.total_nights, b.total_guests, b.adults, b.children, b.special_requests, b.final_amount as booking_total,
                     c.full_name as customer_name, c.email as customer_email, c.phone as customer_phone, c.address as customer_address, c.identity_type, c.identity_number,
                     u.full_name as processed_by
    ...
    $payment = $database->getSingle($query, [$booking_id, $user_id]);
} else {
    $payment = null;
}

if (!$payment) {
    die("Kuitansi / Nota pembayaran tidak ditemukan atau Anda tidak memiliki akses.");
}
...
$total_display = !empty($payment['amount']) ? $payment['amount'] : $payment['booking_total'];
$status_display = !empty($payment['payment_status']) ? strtoupper($payment['payment_status']) : 'CONFIRMED';
?>
<!DOCTYPE html>
<html lang="id">
<head>
...
<div class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4 max-w-800 mx-auto no-print" style="max-width: 800px;">
        <a href="booking_detail.php?id=<?php echo $payment['booking_id']; ?>" class="btn btn-outline-secondary btn-md rounded-pill px-4">
        ...
        <div class="receipt-header d-flex justify-content-between align-items-start">
            ...
            <div class="text-end">
                <span class="badge-paid d-inline-block mb-2">
                    <i class="fas fa-check-circle me-1"></i> <?php echo $status_display; ?>
                </span>
                <div class="fw-extrabold text-dark fs-5" style="font-family: 'Outfit', sans-serif;">NOTA RESMI #<?php echo $payment['id'] ?? $payment['booking_id']; ?></div>
                <small class="text-muted d-block">Tanggal: <?php echo date('d F Y', strtotime($payment['payment_date'] ?? $payment['created_at'])); ?></small>
                <small class="text-muted d-block">Ref: <?php echo htmlspecialchars($payment['transaction_id'] ?? 'TRX-' . str_pad($payment['booking_id'], 6, '0', STR_PAD_LEFT)); ?></small>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <tr>
                            ...
                            <td class="text-center"><?php echo $payment['total_nights']; ?> Malam</td>
                            <td class="text-end">Rp <?php echo number_format($total_display / max(1, $payment['total_nights']), 0, ',', '.'); ?></td>
                            <td class="pe-3 text-end fw-bold">Rp <?php echo number_format($total_display, 0, ',', '.'); ?></td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <div class="row mb-4">
            <div class="col-md-6 mb-3 mb-md-0">
                <div class="p-3 bg-light rounded-3">
                    ...
                    <div class="fw-bold text-dark"><i class="fas fa-credit-card me-1 text-primary"></i> <?php echo strtoupper($payment['payment_method'] ?? 'CASH / TRANSFER'); ?></div>
            ...
            <div class="col-md-6">
                <div class="total-box">
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <span class="fw-semibold">Rp <?php echo number_format($total_display, 0, ',', '.'); ?></span>
                    </div>
                    <div class="d-flex justify-content-between mb-2">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span class="fw-extrabold fs-4 text-success" style="font-family: 'Outfit', sans-serif;">
                            Rp <?php echo number_format($total_display, 0, ',', '.'); ?>
                        </span>
                    </div>
                </div>
            </div>
